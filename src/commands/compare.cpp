...
bool LoadRunSample(const std::string &path, const std::optional<std::string> &expected_mode,
    ...
    static const std::vector<std::string> kCounters = {"cycles",        "instructions", "branches",
...
int HandleCompare(const std::vector<std::string> &args) {
    ...
    const std::vector<std::string> counter_names = {"cycles",        "instructions", "branches",
                                                     "branch_misses", "cache_misses", "page_faults"};
    ...
    if (!json_path.empty()) {
        ...
        json << "{\n"
             ...
             << "  \"inputs\": {\n"
             << "    \"native_files\": " << JsonStringArray(native_paths.empty() ? std::vector<std::string>{
                                                                            native_samples.front().path}
                                                                           : native_paths)
             << ",\n"
             << "    \"qemu_files\": " << JsonStringArray(qemu_paths.empty() ? std::vector<std::string>{
