...
DiagnosisResult DiagnoseRun(const WorkloadRunResult &workload, const PerfStatResult &perf,
    ...
    if (memory_pressure) {
        ...
        result.evidence.push_back({"max_rss_mb", FormatNumber(derived.max_rss_mb, 1),
                                   "Peak RSS sampled from /proc/<pid>/status."});
        if (derived.has_page_fault_rate) {
            result.evidence.push_back({"page_faults_per_sec", FormatNumber(derived.page_fault_rate, 1),
        ...
        result.evidence.push_back({"syscall_time_share", FormatNumber(derived.syscall_share, 3),
                                   "Share of wall time spent inside syscalls."});
        result.evidence.push_back({"io_syscall_share", FormatNumber(derived.io_share, 3),
        ...
        result.evidence.push_back({"syscall_time_share", FormatNumber(derived.syscall_share, 3),
                                   "Share of wall time spent inside syscalls."});
        if (derived.has_top_syscall_share) {
            result.evidence.push_back({"top_syscall_share", FormatNumber(derived.top_syscall_share, 3),
        ...
        result.confidence =
            (derived.ipc >= 1.20 && (!derived.has_syscall_share || derived.syscall_share <= 0.05))
                ? "high"
                : "medium";
        result.evidence.push_back({"ipc", FormatNumber(derived.ipc, 3),
        ...
        if (derived.has_cache_miss_rate) {
            result.evidence.push_back({"cache_miss_per_instruction",
        ...
        result.evidence.push_back({"wall_time_sec", FormatNumber(workload.wall_time_sec, 6),
                                   "Elapsed runtime from fallback timer."});
        result.evidence.push_back({"exit_code", std::to_string(workload.exit_code),
